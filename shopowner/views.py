...
from .models import Notification, Product, Shop, Shopowner, Order
...
@login_required(login_url='partner_with_us/login')
def shop_orders(request, shopid):
    ...
    return render(request, 'shopowner/shop_orders.html', {
        'shop': shop,
        'orders': orders,
        'user': request.user
...
@login_required(login_url='partner_with_us/')
def order(request, orderid):

    order = Order.objects.filter(id=orderid).first()

    if request.method == "POST" and request.POST.get('change_order_status') == 'submit':
    ...
    return render(request, 'shopowner/order_page.html', {
        'user': request.user,
        'order': order
